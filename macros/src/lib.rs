...
#[proc_macro_derive(State, attributes(state_skip, state_read_only, state_derives))]
pub fn derive_state(input: TokenStream) -> TokenStream {
    ...
    let impl_fields = fields.iter().map(|f| {
        ...
        if skip {
            ...
            quote! { #name: ::floem::reactive::create_read_signal(self.#name.clone()) }
        } else {
            quote! { #name: ::floem::reactive::create_rw_signal(self.#name.clone()) }
        }
    });
