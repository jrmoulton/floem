...
impl<'a> AppCommand<'a> {
    fn execute(self) {
        let active = TODOS_STATE.with(|s| s.active);
        let selected = TODOS_STATE.with(|s| s.selected);
        let todos = TODOS_STATE.with(|s| s.todos);
        match self {
            ...
            AppCommand::RefreshDB => {
                TODOS_STATE.with(|s| s.refresh_db());
            }
            AppCommand::NewTodo => {
                TODOS_STATE.with(|s| s.new_todo());
            }
            AppCommand::UpdateDone(todo) => {
                if let Some(db_id) = todo.db_id.with_untracked(|opt_id| *opt_id) {
                    TODOS_STATE.with(|s| s.update_done(db_id, todo.done.get_untracked()));
                } else {
            ...
            AppCommand::UpdateDescription(todo) => {
                if let Some(db_id) = todo.db_id.with_untracked(|opt_id| *opt_id) {
                    TODOS_STATE
                        .with(|s| s.update_description(db_id, &todo.description.get_untracked()));
                } else {
                    AppCommand::CommitTodo(todo).execute();
                }
            }
            AppCommand::CommitTodo(todo) => {
                let new_db_id = TODOS_STATE.with(|s| {
